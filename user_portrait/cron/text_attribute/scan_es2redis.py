...
def scan_es2redis_week():
    ...
    while True:
        try:
            ...
            count += 1
            uid = scan_re['uid']
            update_week_redis.lpush(UPDATE_WEEK_REDIS_KEY, uid)
            if count % 1000 == 0 and count != 0:
                end_ts = time.time()
                print '%s sec count 1000' % (end_ts - start_ts)
                start_ts = end_ts
...
def scan_es2redis_month():
    count = 0
    s_re = scan(es_user_portrait, query={'query':{'match_all': {}}, 'size':1000}, index=por❯
    start_ts = time.time()
    while True:
        try:
            scan_re = s_re.next()['_source']
            count += 1
            uid = scan_re['uid']
            update_month_redis.lpush(UPDATE_MONTH_REDIS_KEY, uid)
            if count % 1000 == 0 and count != 0:
                end_ts = time.time()
                print '%s sec count 1000' % (end_ts - start_ts)
                start_ts = end_ts
